...
for key in VM_STORAGE VM_DATA_STORAGE NETWORK_BRIDGE NETWORK_VLAN GUEST_IPV4 GUEST_GATEWAY DNS_PRIMARY DNS_SECONDARY TLS_CA_REFERENCE BACKUP_TARGET_REFERENCE; do
  value=$(getv "$key")
...
TLS_CA_REFERENCE=$(getv TLS_CA_REFERENCE); BACKUP_TARGET_REFERENCE=$(getv BACKUP_TARGET_REFERENCE)
mkdir -p "$out"
...
echo "- Backup target reference: $BACKUP_TARGET_REFERENCE"
