...
for f in scripts/collect-matriarch-readonly scripts/dispatch-worker scripts/run-worker-headless scripts/wait-worker-result; do sh -n "$root/$f"; done
...
rg -q 'verify_evidence' "$root/scripts/run-worker-headless"
